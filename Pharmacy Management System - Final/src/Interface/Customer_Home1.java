...
public class Customer_Home1 extends javax.swing.JInternalFrame 
{
    ...
    public int checkquantity() /*Checks available quantity for selected item*/
    {
        ...
        else
        {
            ...
            try
            { 
                String checkqty = "SELECT * FROM sales_stock WHERE Item_code = '"+ itcode +"'AND Stock_Type LIKE '%"+ sttype + "%'" ;
    ...
    public void getUpdateStock(String item,String type ,int qty) /* Updates stock details after a transaction*/
    {
        ...
        String sqlselect = "SELECT * FROM sales_stock WHERE Stock_Type =? AND Item_Code=?";
        String sqlupdate = "UPDATE sales_stock SET Quantity = ? WHERE Item_Code=? AND Stock_Type =?";
            ...
            if(rs.next())
            {
                ...
                for(int row = 0; row<rowcount; row++)
                {
                    ...
                    else
                    {
                        ...
                        String a = "INSERT INTO `sales`(`sales_id`, `Item_code`, `sales_qty`, `sales_date`, `Stock_Type`) VALUES ('"+ id +"', '"+itemcode+"' , '"+h+"', '"+ idate +"','"+sttype+"')";
...
//            cardp.setVisible ( true );
    ...
    private void initComponents() {
        ...
        jPanel2.add(jPanel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 1080, 40));
